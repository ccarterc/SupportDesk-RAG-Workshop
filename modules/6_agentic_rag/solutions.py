    ...
    def search_similar_tickets(self, query: str) -> str:
        ...
        if "critical" in query.lower():
            results = self.vectorstore.similarity_search(
                query, k=3, filter={"priority": "Critical"}
            )
        else:
...
Resolved: {ticket["resolved_date"]}"""
    ...
    def search_by_category(self, category: str) -> str:
        ...
        if not category or not category.strip():
            available = sorted({t["category"] for t in self.tickets})
            return (
                f"Error: Please provide a category. Available: {', '.join(available)}"
            )

        category = category.strip()
        # Simple exact category matching (case-insensitive).
        matching = [
            t for t in self.tickets if t["category"].lower() == category.lower()
        ]
    ...
    def search_by_priority(self, priority: str) -> str:
        ...
        if not priority or not priority.strip():
            available = sorted({t["priority"] for t in self.tickets})
            return (
                f"Error: Please provide a priority. Available: {', '.join(available)}"
            )

        priority = priority.strip()
        # Priority filtering supports urgency-focused triage style queries.
        matching = [
            t for t in self.tickets if t["priority"].lower() == priority.lower()
        ]
        ...
        for ticket in matching:
            output += (
                f"• [{ticket['ticket_id']}] {ticket['title']} ({ticket['category']})\n"
            )
...
llm = ChatOpenAI(
    model=os.getenv("OPENAI_CHAT_MODEL", "gpt-5.6-luna"), reasoning_effort="none"
)
...
def run_agent(
    query: str, max_iterations: int = 5, custom_prompt: str | None = None
) -> dict:
    """
...
for test in evaluation_cases:
    ...
    if passed:
        correct += 1
    print(
        f"  {'✓' if passed else '✗'} '{test['query'][:30]}...' → {result['tools_used']}"
    )
