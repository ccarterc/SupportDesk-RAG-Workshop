...
original_splitter = CharacterTextSplitter(
...
chroma_store = Chroma.from_documents(
    documents, embeddings, collection_name="filter_test"
)
...
for category in categories:
    ...
    except Exception:  # noqa: BLE001 - keep the workshop moving if a filter backend fails
...
try:
    ...
    print(f"\nQuery: '{query}'")
    print("Filter: Authentication category + High priority")
    print(f"\nResults ({len(results)}):")
    for doc in results:
        print(
            f"  [{doc.metadata['priority']}] [{doc.metadata['category']}] {doc.metadata['ticket_id']}"
        )
except Exception as e:  # noqa: BLE001 - surface backend-specific filter failures
...
chroma_store = Chroma.from_documents(
    simple_docs, embeddings, collection_name="exercise8", persist_directory=persist_dir
)
...
if os.path.exists("./solution_chroma_db"):
    ...
    if cleaned:
        ...
        print(
            "⚠ Could not fully clean solution_chroma_db (file lock). You can delete it manually later."
        )
