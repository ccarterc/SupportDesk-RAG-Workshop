...
except Exception as e:  # noqa: BLE001 - report any provider or network failure clearly
...
vector_store = Chroma.from_documents(
    documents=documents, embedding=embeddings, collection_name="eval_supportdesk"
)
...
for idx, eval_query in enumerate(eval_queries, 1):
    ...
    except Exception as e:  # noqa: BLE001 - continue evaluating the remaining queries
...
print("  → What % of retrieved documents are actually relevant?")
print("  → Target: > 0.80 for production")

print(f"\nRecall@3:    {avg_recall:.4f}")
print("  → What % of all relevant documents did we find?")
print("  → Target: > 0.70 for production")

print(f"\nF1 Score@3:  {avg_f1:.4f}")
print("  → Balanced measure of retrieval quality")
print("  → Target: > 0.75 for production")
...
Reasoning: <explanation>"""
    ...
    except Exception as e:  # noqa: BLE001 - convert judge failures into an error score
...
print("  → Are answers supported by retrieved context?")
print("  → Target: > 0.80 (minimize hallucinations)")

print(f"\nCompleteness:  {avg_completeness:.4f}")
print("  → Do answers fully address the questions?")
print("  → Target: > 0.75 (comprehensive responses)")
...
if avg_groundedness < 0.70:
    ...
    print(
        "  Fix: Stricter grounding prompt, require ticket citations, refuse when unsupported."
    )
        ...
        for query in queries[:3]:  # Test subset
            try:
                ...
                metrics = calculate_retrieval_metrics(
                    retrieved_ids, query["relevant_ticket_ids"], k=k
                )
                retrievals.append(metrics)
            except Exception as e:  # noqa: BLE001 - continue testing other configurations
                print(f"\n    ⚠ Error: {str(e)[:50]}")
                continue
...
for config, metrics in comparison.items():
    print(
        f"{config:<10} {metrics['precision']:<12.4f} {metrics['recall']:<12.4f} {metrics['f1']:<12.4f}"
    )
